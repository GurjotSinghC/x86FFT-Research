...
FlexibleDataBuffer:	.space	32000
...
file.readfile:	
	...
	sw	$v0,m	#this is the power of two value

	mtc1	$v0,$f0			#v0 holds num of bytes need to do 
	cvt.w.d	$f0,$f0
	...
	li	$t0,m
	li	$t1,2
	#this loop created the full power of t 2^m = N
1:	beqz	$t0,end
	mul	$t1,$t1,$t1
	addi	$t0,-1
	j	1b
...
# 	s.d	$v0,phiT		# this si the result of phit * phit
	...
	mtc1	$f0,$t6
	cvt.w.d	$f0,$f0
	s.d	$f0,complex.real($t7)
	
	li	$t6,0

	mtc1	$f0,$t6
	cvt.w.d	$f0,$f0
	s.d	$f0,complex.imaginary($t7)

	#did this in data section#         T = 1.0L;
	li	$t9,0 #l =0	
				
tloop:	bge	$t9,$t1,tloopend#         for (unsigned int l = 0; l < k; l++)
